...
if ( ! function_exists( 'wolmart_single_product_vendor_products' ) ) {
	function wolmart_single_product_vendor_products( $sp_type = '', $show_count = 8 ) {
		...
		} else if ( class_exists( 'WCV_Vendors' ) ) {
			$vendor_id = get_post_field( 'post_author', $product_id );
		...
		} else if ( defined( 'MVX_PLUGIN_VERSION' ) ) {
		...
		<div class="wolmart-same-vendor-products-wrapper <?php echo ( $sp_type == 'gallery' ? 'gallery-type' : '' ); ?>">
			<h4 class="wolmart-same-vendor-products-title"><?php esc_html_e( 'More items from this seller'); ?><a href="<?php echo esc_url( $store_url ); ?>"><?php esc_html_e( 'View All', 'wolmart' ); ?></a></h4>
